        ...
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    {
                        case ('1'):                           
                            page = phoneBook.OrderBy(s => s.Name).ThenBy(s => s.LastName).Take(2);
                            break;
                        case ('2'):
                            page = phoneBook.OrderBy(s => s.Name).ThenBy(s => s.LastName).Skip(2).Take(2);
                            break;
                        case ('3'):
                            page = phoneBook.OrderBy(s => s.Name).ThenBy(s => s.LastName).Skip(4).Take(2);
                            break;
                    }
